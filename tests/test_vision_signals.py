...
def test_orb_without_references_is_zero():
    rng = np.random.default_rng(12)
    img = rng.integers(0, 255, size=(80, 80), dtype=np.uint8)
    assert s.match_orb(img, []) == 0.0
    assert s.match_orb(img, [None]) == 0.0


def test_orb_handles_empty_roi():
    assert s.match_orb(None, [np.zeros((10, 32), np.uint8)]) == 0.0


@pytest.mark.parametrize("value", [0, 255])
def test_orb_on_featureless_image_is_zero(value):
    flat = np.full((80, 80), value, dtype=np.uint8)
    assert s.match_orb(flat, [np.zeros((10, 32), np.uint8)]) == 0.0
