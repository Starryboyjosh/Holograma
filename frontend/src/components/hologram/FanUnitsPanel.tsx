...
function FanUnitCard({ role, unit, onSave, onAction }: { role: FanRole; unit?: FanUnitStatus; onSave: (role: FanRole, payload: UnitUpdate) => Promise<boolean>; onAction: (role: FanRole, action: 'connect' | 'disconnect' | 'identify' | 'test', index?: number) => Promise<boolean> }) {
  ...
  return <article className="rounded-[30px] border border-gray-200 p-4" aria-label={ROLE_LABELS[role]}>
    ...
    <div className="flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={() => void save()} className="rounded-[50px] bg-orange px-3 py-2 text-xs font-bold text-white disabled:opacity-50">Guardar</button><button type="button" disabled={busy || !unit?.connected} onClick={() => void action('identify')} className="rounded-[50px] border px-3 py-2 text-xs font-bold disabled:opacity-40">Identificar</button><button type="button" disabled={busy} onClick={() => void action(unit?.connected ? 'disconnect' : 'connect')} className="rounded-[50px] border px-3 py-2 text-xs font-bold disabled:opacity-40">{unit?.connected ? 'Desconectar' : 'Conectar'}</button><button type="button" disabled={busy || !unit?.connected || index < 0 || index > 255} onClick={() => void action('test')} className="rounded-[50px] border px-3 py-2 text-xs font-bold disabled:opacity-40">Probar</button></div>
  </article>;
}
